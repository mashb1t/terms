<div>
    ...
    @endif
    @if($showCreateButton)
        <x-jet-button class="ml-2" wire:click="edit(null,{{ $quiz->id ?? null }})" wire:loading.attr="disabled">
            {{ __('Create') }}
        </x-jet-button>
    @endif
    <div class="relative">
    ...
    <x-jet-dialog-modal wire:model="showDeleteModal">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="deleteConfirmed()" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-jet-danger-button>
        </x-slot>
    </x-jet-dialog-modal>
